...
- [ ] FoldedTaskSignature over the signature dimensions
- [ ] PolicyGraph: signature -> action-value (conclusive rewards only), warm-start transfer
- [ ] SkipXPolicy (skip:planner/retrieval/reviewer by signature; never skip verifier high-risk)
- [ ] CrossJudgeRewardAudit can change a promotion

## Alpha 29 — real-repo replay (escape synthetic)
- [ ] RepoReplayDataset: real-world-shaped historical bugs (pre-fix/known-fix/hidden tests)
- [ ] LIVE: run OpenAI harness over replay tasks; tier=real_repo_replay
