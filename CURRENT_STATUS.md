...
Last updated: 2026-06-07 (Round 28 + evaluation follow-ups + repo-map feature + Alpha 42 MetaRouter + Alpha 43 Scale-Proven + Alpha 44 Hard-Realism MetaRouter + Heterogeneity Round — live cross-provider arena across Anthropic haiku/sonnet/opus AND Google Gemini 3.x flash-lite/flash/pro, closing the #1 gap of real model/provider heterogeneity; see `reports/heterogeneity_report.md` + Harness Round — live routing across agent HARNESSES not just models: single-shot vs in-process Claude tool-loop vs the real Gemini CLI agent vs OpenHands V1 local runtime, autonomy rescues context-gated tasks single-shot fails 0/8, see `reports/harness_routing_report.md` + Verifier+Router Round — Phase 1 independent proxy verifier lifts candidate selection 0.79->1.00 at +7% cost (precision=recall=1.0 vs hidden); Phase 2 unified router composes memory+FinOps+safe escalation and, with memory, converges to the optimal per-family cost over sessions while a memoryless router stays ~35% dearer, see `reports/verifier_and_router_report.md` + Enhancements-v2 (4 research-grounded increments) — router now spans model×thinking-level×harness with Gemini CLI/OpenHands live and Codex/Claude-Code availability-gated; aging-aware memory survives a repo migration-and-back where naive memory regresses; offline-learned ladder is 19.4% cheaper at equal success; self-evolving repo playbook + calibrated no-false-commit stop, see `reports/enhancements_v2_report.md`). Tests: 1335 passing, 18 skipped on a fresh checkout with the `learning` extra (1293 passing / 22 skipped on a bare `dev`-only checkout); skips are optional deps/daemons that self-skip when absent (docker/openhands/openai-sdk/pgvector/qdrant/skillopt) — see `reports/pytest.txt`;
`ruff` + `mypy` clean across 340 source files; see `reports/coverage.txt`. Gate:
