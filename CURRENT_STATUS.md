...
Last updated: 2026-06-07 (Round 28 + evaluation follow-ups + repo-map feature + Alpha 42 MetaRouter Arena/FinOps — guarded PRs, operator inbox, xdist). Tests: 1235 passing, 18 skipped on a fresh checkout with the `learning` extra (1231 passing / 22 skipped on a bare `dev`-only checkout); skips are optional deps/daemons that self-skip when absent (docker/openhands/openai-sdk/pgvector/qdrant/skillopt) — see `reports/pytest.txt`;
`ruff` + `mypy` clean across 312 source files; see `reports/coverage.txt`. Gate:
