...
def test_should_repair_only_when_all_failed_and_budget_remains() -> None:
    ...
    # depth exhausted
    assert not should_repair(all_failed=True, depth=1, max_depth=1, spent=0.0, budget=0.1)
    # budget exhausted
    assert not should_repair(all_failed=True, depth=0, max_depth=1, spent=0.2, budget=0.1)
