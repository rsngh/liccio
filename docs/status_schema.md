...
| **vendor harness** | A wrapper around an external vendor coding-agent SDK/CLI (e.g. Claude Agent SDK, OpenHands, Codex CLI) run as a black box, normalized into the same `AgentTrace` surface. Implemented as capability-gated shims (`claude_agent_sdk`, `codex_cli`) on `VendorHarnessAdapter`; they self-report `unavailable` without the SDK/binary/key. Distinct from an ACP true harness. |
