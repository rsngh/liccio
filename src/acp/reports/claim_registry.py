...
CLAIMS: list[Claim] = [
    ...
    # --- Alpha 44 ---
    Claim("hard_arena_bucket_win",
          "on hard tasks, a metarouting policy beats cheap_single with bucket-level CI separation",
          requires=("reports/hard_realism_arena.json",),
          tier="any", must_assert=(("bucket_level_ci_separated_win", True),)),
    Claim("memory_lifespan",
          "memory improves repeated-pattern tasks over a long-lived project history",
          requires=("reports/memory_lifespan_benchmark.json",),
          tier="any", must_assert=(("revised_after_migration", True),)),
    Claim("context_embedding_tested",
          "grep-vs-embedding is measured, not assumed (embedding tested honestly)",
          requires=("reports/context_experiment_matrix.json",),
          tier="any", must_assert=(("embedding_tested_honestly", True),)),
    Claim("issue_replay_honest",
          "issue-replay bundles are clearly labeled (frozen synthetic, not real-ingest claims)",
          requires=("reports/issue_replay_bundles.json",),
          tier="any", must_assert=(("all_offline_fair", True),)),
]
